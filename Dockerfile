...
EXPOSE 8089
ENTRYPOINT [ "java", "-jar"]
CMD ["/kustvakt/Kustvakt.jar", ""]

# docker build -f Dockerfile -t korap/kustvakt:{nr} .
# docker build -f Dockerfile -t korap/example-index:{nr} --target example-index .
# run full version
# docker run --rm --net host -v [absolute-path-sample-index]:/kustvakt/index -it korap/kustvakt:{nr}
# run lite version 
# docker run --rm --net host -v [absolute-path-sample-index]:/kustvakt/index -it korap/kustvakt:{nr} /kustvakt/Kustvakt.jar --lite