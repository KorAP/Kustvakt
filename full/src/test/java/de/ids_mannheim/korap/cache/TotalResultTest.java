...
public class TotalResultTest extends SpringJerseyTest {
    ...
    private void testSearchWithCutOffFalse () throws KustvaktException {
        String query = "{\"meta\":{\"startPage\":1,\"tokens\":false,\"snippets\":true,"
                + "\"cutOff\":false,\"timeout\":10000},\"query\":{\"@type\":\"koral:"
                + "token\",\"wrap\":{\"@type\":\"koral:term\",\"match\":\"match:eq\","
                + "\"key\":\"die\",\"layer\":\"orth\",\"foundry\":\"opennlp\","
                + "\"rewrites\":[{\"@type\":\"koral:rewrite\",\"src\":\"Kustvakt\","
                + "\"operation\":\"operation:injection\",\"scope\":\"foundry\"}]}},"
                + "\"@context\":\"http://korap.ids-mannheim.de/ns/koral/0.3/context."
                + "jsonld\",\"collection\":{\"@type\":\"koral:doc\",\"match\":"
                + "\"match:eq\",\"type\":\"type:regex\",\"value\":\"CC-BY.*\",\"key\":"
                + "\"availability\",\"rewrites\":[{\"@type\":\"koral:rewrite\",\"src\":"
                + "\"Kustvakt\",\"operation\":\"operation:insertion\",\"scope\":"
                + "\"availability(FREE)\"}]}}";
        
        int cacheKey = searchService.createTotalResultCacheKey(query);
        String updatedQuery = searchService
                .precheckTotalResultCache(cacheKey, query);

        JsonNode node = JsonUtils.readTree(updatedQuery);
