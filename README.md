...
Build Kustvakt full image using the Dockerfile

```
docker build -f Dockerfile -t korap/kustvakt-full-[version] --target kustvakt-full .
```

Build Kustvakt lite image using the Dockerfile

```
docker build -f Dockerfile -t korap/kustvakt-lite-[version] --target kustvakt-lite .
``` 
